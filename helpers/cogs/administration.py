...
class Administration:
    ...
    @commands.command(name='on-join')
    async def on_join_message(self, ctx, *, message):
        await self.dbm.set_on_join_message(message, ctx.channel.guild)
        await ctx.send('Updated this guild\'s on join message!')

    @commands.guild_only()
    @commands.has_permissions(administrator=True)
    @commands.command(name='default-role')
    async def on_join_role(self, ctx, *, role_name):
        status = await self.dbm.set_on_join_role(role_name, ctx.channel.guild)
        print(role_name)
        if status:  # Success
    ...
    @commands.guild_only()
    # @commands.has_permissions(administrator=True)
    @commands.command(name='default-realm')
    async def default_realm(self, ctx, *, realm):
        status = await self.dbm.set_default_realm(ctx.channel.guild, realm)
        if status:  # Success
            await ctx.send(f'Updated this guild\'s default WoW realm to `{realm.title()}`!')
        else:  # Failed
            await ctx.send('Something broke :(')

    @commands.guild_only()
    # @commands.has_permissions(administrator=True)
    @commands.command(name='default-region')
    async def default_region(self, ctx, *, region):
        supported = ['EU', 'US', 'KR', 'TW']
        if region.upper() not in supported:
            await ctx.send(
                'Region `{}` is not supported.\nSupported regions are: {}'.format(region, ', '.join(supported))
            )
        status = await self.dbm.set_default_region(ctx.channel.guild, region)
        if status:  # Success
            await ctx.send(f'Updated this guild\'s default WoW region to `{region.upper()}`!')
        else:  # Failed
            await ctx.send('Something broke :(')
    ...
    async def settings(self, ctx):
        default_realm = await self.dbm.get_default_realm(ctx.guild)
        default_region = await self.dbm.get_default_region(ctx.guild)
        await ctx.send(f'Default server realm is {default_realm.title()}({default_region.upper()})')
